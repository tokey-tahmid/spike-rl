...
"""
How to run:
requirements after creating pyframework environment: torch, gym, mujoco==2.3.3, numpy==1.24.0, mpi4py
...
def spikeRL_train(env_fn, actor_critic=SpikingAC_Network, ac_kwargs=dict(), seed=0,
    ...
    # Adjust seed by rank to ensure different random seeds
    seed += 10000 * rank

    # Set device
    if use_cuda and torch.cuda.is_available():
        num_gpus = torch.cuda.device_count()
        # Assign a GPU to each process based on rank
    ...
    # Synchronize initial parameters across all processes
    def sync_params(params):
        for param in params:
            p_numpy = param.data.cpu().numpy()
            comm.Bcast(p_numpy, root=0)
            param.data.copy_(torch.from_numpy(p_numpy).to(param.device))

    sync_params(ac.parameters())
    sync_params(ac_targ.parameters())
    ...
    # Function to average gradients across processes
    def mpi_avg_grads(parameters):
        for param in parameters:
            if param.grad is not None:
                grad_numpy = param.grad.detach().cpu().numpy()
                avg_grad = np.zeros_like(grad_numpy)
                comm.Allreduce(grad_numpy, avg_grad, op=MPI.SUM)
                avg_grad /= size
                param.grad.data.copy_(torch.from_numpy(avg_grad).to(param.grad.device))
    ...
    def update(data, timer):
        ...
        if timer % policy_delay == 0:
            ...
            scaler.unscale_(spikeRL_optimizer)
            mpi_avg_grads(ac.spikeRL.parameters())
