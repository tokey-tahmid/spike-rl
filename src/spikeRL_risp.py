...
"""
def build_neuro_network(spike_mlp, proc):
    net = neuro.Network()
    net.set_properties(proc.get_network_properties())

    # Create nodes for input, hidden, output, and bias layers
    node_id = 0
    node_map = {}  # Map PyTorch layer indices to node IDs in neuro

    # Input layer
    input_size = spike_mlp.in_pop_dim
    for i in range(input_size):
        node = net.add_node(node_id)
        node.set("Threshold", 0.75)
        net.add_input(node_id)
        node_map[f"input_{i}"] = node_id
        node_id += 1

    # Bias node
    bias_node_id = node_id
    bias_node = net.add_node(bias_node_id)
    bias_node.set("Threshold", -1)  # Always active
    net.add_input(bias_node_id)
    node_map["bias"] = bias_node_id
    node_id += 1

    # Hidden layers
    for layer_idx, layer_size in enumerate(spike_mlp.hidden_sizes):
        for i in range(layer_size):
            node = net.add_node(node_id)
            node.set("Threshold", 0.75)
            node_map[f"hidden_{layer_idx}_{i}"] = node_id
            node_id += 1

    # Output layer
    output_size = spike_mlp.out_pop_dim
    for i in range(output_size):
        node = net.add_node(node_id)
        node.set("Threshold", 0.75)
        net.add_output(node_id)
        node_map[f"output_{i}"] = node_id
        node_id += 1

    # Create edges (synapses) based on the weights in SpikeMLP
    # Input to first hidden layer
    prev_layer_size = input_size
    prev_layer_key = "input"
    for layer_idx, layer_size in enumerate(spike_mlp.hidden_sizes):
        weight_matrix = spike_mlp.hidden_layers[layer_idx].weight.data.cpu().numpy()
        bias_vector = spike_mlp.hidden_layers[layer_idx].bias.data.cpu().numpy()
        for i in range(layer_size):
            post_node_id = node_map[f"hidden_{layer_idx}_{i}"]
            # Add bias edge
            edge = net.add_edge(node_map["bias"], post_node_id)
            edge.set("Weight", bias_vector[i])
            edge.set("Delay", 1)
            for j in range(prev_layer_size):
                pre_node_id = node_map[f"{prev_layer_key}_{j}"]
                edge = net.add_edge(pre_node_id, post_node_id)
                weight = weight_matrix[i][j]
                edge.set("Weight", weight)
                edge.set("Delay", 1)
        prev_layer_size = layer_size
        prev_layer_key = f"hidden_{layer_idx}"

    # Hidden to output layer
    weight_matrix = spike_mlp.out_pop_layer.weight.data.cpu().numpy()
    bias_vector = spike_mlp.out_pop_layer.bias.data.cpu().numpy()
    for i in range(output_size):
        post_node_id = node_map[f"output_{i}"]
        # Add bias edge
        edge = net.add_edge(node_map["bias"], post_node_id)
        edge.set("Weight", bias_vector[i])
        edge.set("Delay", 1)
...
def print_net_properties(proc):
    props = proc.get_network_properties()

    print("Threshold type:", props.nodes["Threshold"].type)
    print("Minimum threshold value:", props.nodes["Threshold"].min_value)
    print("Maximum threshold value:", props.nodes["Threshold"].max_value, "\n")

    print("Delay type:", props.edges["Delay"].type)
    print("Minimum delay value:", props.edges["Delay"].min_value)
    print("Maximum delay value:", props.edges["Delay"].max_value, "\n")

    print("Weight type:", props.edges["Weight"].type)
    print("Minimum weight value:", props.edges["Weight"].min_value)
    print("Maximum weight value:", props.edges["Weight"].max_value, "\n")
...
def spikeRL_train(env_fn, actor_critic=SpikingAC_Network, ac_kwargs=dict(), seed=0,
              ...
              polyak=0.995, learning_rate=1e-4, q_lr=1e-3, batch_size=100, start_steps=500,
              update_after=500, update_every=50, act_noise=0.1, target_noise=0.2,
              noise_clip=0.5, policy_delay=2, num_test_episodes=50, max_ep_len=100,
    ...
    # Build the network using neuro
    spike_mlp = ac.spikeRL.snn  # Assuming 'ac' is the actor-critic network instance
    net, node_map = build_neuro_network(spike_mlp, proc)

    # Load the network onto the processor
    proc.load_network(net)

    print("Network properties before training:")
    print_net_properties(proc)
    ...
    def run_simulation(proc, obs, spike_encoder, batch_size, node_map):
        # Encode the observation into spike trains
        in_pop_spikes = spike_encoder(obs, batch_size)
        # Apply spikes to the processor
        for step in range(spike_encoder.spike_ts):
            spikes = []
            # Apply bias spike
            bias_spike = neuro.Spike(id=node_map["bias"], time=step, value=1)
            spikes.append(bias_spike)
            for i in range(spike_encoder.encoder_neuron_num):
                if in_pop_spikes[0][i][step] > 0:
                    spike = neuro.Spike(id=i, time=step, value=in_pop_spikes[0][i][step].item())
                    spikes.append(spike)
            proc.apply_spikes(spikes)
        # Run the processor
        proc.run(spike_encoder.spike_ts)
        # Get the output spikes
        output_spikes = proc.output_vectors()
        # Process the output spikes to get the action
        # Assuming we average the spikes over time
        out_pop_activity = []
        for spikes in output_spikes:
            out_pop_activity.append(len(spikes) / spike_encoder.spike_ts)
        out_pop_activity = torch.tensor(out_pop_activity, device=obs.device)
        # Decode the action
        action = ac.spikeRL.decoder(out_pop_activity.unsqueeze(0))
        action = ac.spikeRL.act_limit * action
    ...
    def compute_loss_q(data):
        ...
        loss_info = dict(Q1Vals=q1.to('cpu').detach().numpy(),
    ...
    def compute_loss_spikeRL(data):
        o = data['obs']
        q1_spikeRL = ac.q1(o, ac.spikeRL(o, batch_size))
    ...
    def update(data, timer):
        ...
        if timer % policy_delay == 0:
            ...
            loss_spikeRL.backward()
            spikeRL_optimizer.step()
        ...
        net, node_map = build_neuro_network(ac.spikeRL.snn, proc)
        proc.load_network(net)

    def get_action(obs, noise_scale):
        # Normalize observation
        norm_obs = replay_buffer.normalize_obs(obs)
        obs_tensor = torch.as_tensor(norm_obs, dtype=torch.float32, device=device).unsqueeze(0)
        # Get action from risp processor
        a = run_simulation(proc, obs_tensor, ac.spikeRL.encoder, batch_size=1, node_map=node_map)
    ...
    print("Running SpikeRL TD3 Algorithm")
    ...
    for t in range(total_steps):
        ...
        if t >= update_after and t % update_every == 0:
            ...
            # Rebuild and reload the network
            net, node_map = build_neuro_network(ac.spikeRL.snn, proc)
            proc.load_network(net)
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            last_fires = proc.neuron_last_fires()
            print("last fires:",last_fires)
            counts = proc.neuron_counts()
            print("counts", counts)
            v = proc.neuron_vectors()
            print("neuron vector", v)
